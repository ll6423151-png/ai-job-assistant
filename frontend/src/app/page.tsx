...
export default function Home() {
  ...
  return <main className="app-shell min-h-screen px-5 py-6 text-ink sm:px-8 sm:py-8"><div className="mx-auto max-w-7xl">
    ...
    <div className="app-content grid gap-8 py-8 lg:grid-cols-[minmax(0,1fr)_240px]"><div className="min-w-0">{panels[activeTab]}</div><aside className="app-status border-l border-slate-200 pl-0 lg:pl-7"><h2 className="text-base font-semibold">运行边界</h2><div className="mt-4 border-y border-slate-200 py-4"><strong className="text-lg text-emerald-700">核心工作台可用</strong><p className="mt-1 text-sm text-muted">外部平台能力取决于当前连接</p></div><div className="mt-5 space-y-3 text-xs leading-5 text-muted"><p>账号数据隔离已启用。</p><p>最终提交必须逐条确认。</p><p>智联自动化需连接本机浏览器桥接。</p><p>登录、验证码和平台风控由用户处理。</p></div></aside></div>
  </div></main>;
}
